...
public class LevelTransition : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (Player.GetComponent<PlayerMover>().hasParashute && isLevel == 0)
            {
                LoadLevel.GetComponent<Loadlevel>().Level2();
            }
            if (Player.GetComponent<PlayerMover>().hasJetPack && isLevel == 1)
            {
                LoadLevel.GetComponent<Loadlevel>().Level3();
            }
            else if (!Player.GetComponent<PlayerMover>().hasJetPack && isLevel == 1)
            {
